...
This describes `litmus-v1` as specified. Per the repo [`README.md`](./README.md), the harness and onchain code are **still being built** — today's public grades are metadata-only *adoption* scoring in `packages/scoring`. The behavioral harness is designed to ship open-source as `@polygraph/probes` (run via `npx polygraphso litmus <server>`); the package layout and build sequence are in [`technical-design.md`](./technical-design.md).
