...
export const RUN_SKILL_LITMUS_TOOL_DESCRIPTION = [
  `Grade a Claude Code / Agent Skill A/B/D/F against the open static safety litmus (${SKILL_METHODOLOGY_VERSION}): a byte scan, no execution, so an A is static-clean, not behavioral proof.`,
  "Prefer verify_skill_attestation for an already-published grade; use this",
  "for a local or public GitHub skill directly.",
  "",
  "Scans SKILL.md plus any bundle for S-01 prompt-injection, S-03 data-",
  "exfiltration instructions, and S-04 dangerous bundled-script commands, then",
  "content-hashes the whole directory. A command built or fetched at runtime",
  "is not visible to static scanning.",
  "",
  "Also returns an advisory quality signal (well-formed/issues/malformed,",
  "never an A-F letter): deterministic checks always run; LLM-judged axes run",
  "only when a judge is available (host-model sampling or a configured key).",
  "",
  "skill_ref: a local skill directory containing SKILL.md, or a public GitHub",
  "skill (a github.com URL or github/<owner>/<repo>#<path>), downloaded over",
  "TLS and scanned locally.",
].join("\n");
