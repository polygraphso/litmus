...
import { parseSinkholeOutput, egressCanaryFindings, hostDnatCommands, type EgressAttempt } from "./egress-runner.js";

/**
 * Live, Docker-gated proof that the litmus-v4 HOST-DNAT gateway capture closes the
 * IP-literal / DoH blind spot (litmus-test §7) that the legacy `--internal` capture
 * misses. Opt-in (slow — builds an image + runs containers + touches host iptables;
 * needs a real Linux + iptables host):
 *
 ...
 * in the plaintext request line. Under `--internal` the packet is dropped at routing
 * and the sink sees nothing (the false "no egress" pass). Under host-DNAT the host
 * redirects the off-subnet egress to the sink regardless of DNS — and the canary
 * landing at the SINK (not the real host) is itself the zero-real-egress proof.
 */
...
// A REAL, routable IP literal: if the redirect failed and the packet escaped, it
...
/** Run a fixed iptables script in the HOST netns via an ephemeral NET_ADMIN helper. */
async function hostIptables(cmds: string[]) {
  await d(["run", "--rm", "--network", "host", "--cap-add=NET_ADMIN", "--cap-drop=ALL", "--entrypoint", "sh", IMAGE_TAG, "-c", cmds.join("; ")]).catch(() => {});
}
...
describe.skipIf(process.env.LITMUS_DOCKER_TESTS !== "1")("egress IP-literal capture — live (Docker-gated)", () => {
  it("host-DNAT CAPTURES an IP-literal phone-home that --internal mode MISSES", async () => {
    if (!(await hasDocker())) {
    ...
    expect(egressCanaryFindings(internalAttempts, [aCanary])).toHaveLength(0); // the blind spot

    // ── Phase B: host-DNAT — the IP literal IS captured at the sink ──
    const bSuffix = randomUUID().slice(0, 8);
    const bNet = `pg-iplit-gw-${bSuffix}`;
    const bSink = `pg-iplit-sink-${bSuffix}`;
    const bCanary = `POLYGRAPH-CANARY-hostdnat-${bSuffix}`;
    let bridge = "";
    let scope: { bridge: string; subnet: string; sinkIp: string } | null = null;
    let gatewayAttempts: EgressAttempt[] = [];
    try {
      await d(["network", "create", "-o", "com.docker.network.bridge.enable_ip_masquerade=false", bNet]);
      const netId = (await d(["network", "inspect", "-f", "{{.Id}}", bNet])).trim();
      bridge = `br-${netId.slice(0, 12)}`;
      const subnet = (await d(["network", "inspect", "-f", "{{(index .IPAM.Config 0).Subnet}}", bNet])).trim();
      ...
      await new Promise((r) => setTimeout(r, 1500));
      scope = { bridge, subnet, sinkIp: bSinkIp };
      // Host rules added BEFORE the target runs — no un-DNATed egress window.
      await hostIptables(hostDnatCommands("I", scope));
      // Target runs NORMALLY (own netns, cap-dropped, --dns sink) — no route tricks.
      await d(["run", "--rm", "--network", bNet, "--dns", bSinkIp, "--user", "node", "--cap-drop=ALL", "--entrypoint", "node", IMAGE_TAG, "-e", literalProbeJs(bCanary)]).catch(() => {});
      await new Promise((r) => setTimeout(r, 800));
      gatewayAttempts = parseSinkholeOutput(await d(["logs", bSink]));
    } finally {
      if (scope) await hostIptables(hostDnatCommands("D", scope)); // remove HOST rules first
