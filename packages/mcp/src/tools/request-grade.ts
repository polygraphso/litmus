...
export const REQUEST_GRADE_TOOL_DESCRIPTION = [
  "Queue an MCP server for grading with polygraph.so; use this when",
  "check_server returns not_available and you don't want to run_litmus",
  "yourself.",
  "",
  "Recording the request is free; grading starts once its one-time fee (about",
  "$1) is paid, and the grade publishes within 48 hours of payment. Pay at the",
  "returned payUrl ($POLYGRAPH), or, for x402-capable clients, POST the same",
  "request to the returned x402Url with an X-PAYMENT header (USDC on Base). No",
  "contact details needed; requesting the same server twice is a no-op, not a",
  "duplicate. Read the result later with check_server.",
  "",
  "Input: server_ref, the same registry-prefixed identifier check_server takes",
  "(npm/@scope/name, pypi/name, github/owner/repo).",
  "",
  "Returns { status: 'queued', created, demand, requestId, payment }: created",
  "is false if already recorded, demand is how many requests stand behind it,",
  "payment.required says whether the fee is still owed.",
].join("\n");
